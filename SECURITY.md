...
Backup содержит данные БД и полную поддерживаемую безопасную часть collector/backup/runtime-настроек, шифруется PHB3 до отправки и не включает ключ шифрования, Telegram-токен, строку БД либо административный ключ. Текущий manifest v4 требует все settings-файлы и обе истории operational audit, а restore отклоняет любую ZIP-запись вне фиксированной схемы, поэтому ложное `secretsIncluded=false` не может скрыть дополнительный файл с секретами. ZIP передаётся шифратору через bounded memory pipe и никогда не записывается на диск в открытом виде; после аварийного завершения следующий backup под cluster lock удаляет `.partial`, Telegram-part и legacy plaintext ZIP от прежних версий. Потеря ключа восстановления необратима.
