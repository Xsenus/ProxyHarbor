...
Backup содержит данные БД и полную поддерживаемую безопасную часть collector/backup/runtime-настроек, шифруется PHB3 до отправки и не включает ключ шифрования, Telegram-токен, строку БД либо административный ключ. Текущий manifest v5 требует все settings-файлы и обе истории operational audit, а restore отклоняет любую ZIP-запись вне фиксированной схемы, поэтому ложное `secretsIncluded=false` не может скрыть дополнительный файл с секретами. ZIP передаётся шифратору через bounded memory pipe и никогда не записывается на диск в открытом виде; после аварийного завершения следующий backup под cluster lock удаляет `.partial`, Telegram-part и legacy plaintext ZIP от прежних версий. Потеря ключа восстановления необратима.

При повторе Telegram-доставки HTTP response, multipart и файловый поток закрываются до ожидания `retry_after`; внешний сервис не может удерживать handle зашифрованного архива до 30 секунд каждой паузы.
...
Release workflow имеет deny-by-default permissions и выдаёт `packages`, `attestations`, OIDC либо `contents:write` только соответствующему job. Он принимает только строгий SemVer tag, публикует отдельные multi-architecture GHCR manifests с SBOM/BuildKit provenance и фиксирует их digest в release manifest. Публичные репозитории дополнительно получают подписанную Sigstore provenance через GitHub Artifact Attestations; private attestation включается только оператором на поддерживаемом Enterprise Cloud плане. `latest` никогда не указывает на prerelease. CI запрещает внешние actions без полного commit SHA и проверяет, что release Compose действительно не выполняет локальную сборку.
...
Основной Compose-контур также не передаёт PostgreSQL password, admin key, backup encryption key и Telegram credentials как container environment. PostgreSQL использует нативный `POSTGRES_PASSWORD_FILE`, API и restore — общий bounded `SecretFiles__*` loader с максимальным размером 16 КиБ, строгим UTF-8 и запретом relative path/control characters. Файлы монтируются только сервисам-потребителям. CI проверяет отсутствие фактических значений в rendered Compose и `docker inspect`, точное число secret mounts, а затем выполняет реальный backup/restore через эти mounts.
