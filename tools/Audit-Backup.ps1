...
$report = [ordered]@{
    ...
    telegramRequired = -not $AllowLocalOnly
    success = $false
    created = $null
    sizeBytes = 0
    startedAt = $null
...
try {
    ...
    if (-not $AllowLocalOnly -and -not $report.sentToTelegram) {
        throw 'Backup создан локально, но обязательная доставка в Telegram не подтверждена.'
    }
    ...
    $report.success = $true
    Write-Host ("Backup-аудит пройден: {0}, {1} байт, Telegram={2}." -f
        $report.created, $report.sizeBytes, $report.sentToTelegram) -ForegroundColor Green
}
catch {
