# Каталог провайдеров бесплатных прокси

В `BuiltInSourceCatalog` входят 543 feed от 283 независимых provider identities. Документ предназначен оператору; публичная главная страница источники не раскрывает. Таблица ниже описывает исходное ядро каталога, а полный канонический перечень URL, рангов и протоколов хранится в коде.
...
10 сентября 2026 года production-аудит проверил состояние всех встроенных feed. Девять проблемных proxy-источников заменены: пустой, HTML-страница, локальные loopback-порты, ошибочно классифицированная VPN-подписка и пять файлов больше безопасного лимита 10 МБ. Два исчезнувших country-feed Au1rxx удалены из VPN-каталога, а MetaFetch перенесён туда как VPN subscription. Новые URL проверены на HTTP 200 и непустой разбираемый ответ. Полный collector/validation прогон остаётся обязательным release-gate и не подменяется URL-проверкой.
...
Бесплатные прокси меняются каждую минуту, поэтому нельзя честно обещать, что все строки внешнего feed будут рабочими. Гарантия сервиса другая и проверяемая: все 283 provider identities входят в сбор; неработающие адреса не публикуются; отказавший, пустой или усечённый feed отражается в diagnostics, Prometheus и еженедельном CI-аудите.

Все 543 URL, их протоколы и эксплуатационный порядок находятся в [`BuiltInSourceCatalog.cs`](../src/ProxyHarbor.Infrastructure/BuiltInSourceCatalog.cs). Команда воспроизводимого production-аудита описана в [`SOURCES.md`](SOURCES.md).
