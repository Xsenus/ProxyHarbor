...
Последняя прямая повторная проверка доступности выполнена **10 августа 2026 года в 10:32 (Asia/Novosibirsk)**: **81/81** endpoint вернули HTTP 200 и непустые данные с распознаваемыми `IP:port`, всего 892 868 строк-кандидатов (17 045 830 байт). Предшествующий полный production-аудит в 07:59 пропустил те же 81/81 ленты через настоящий collector, ограничения размера, parser и межисточниковую дедупликацию без ошибок, пустых результатов или усечения.
