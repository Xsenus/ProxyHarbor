...
Встроенный каталог содержит **543 proxy-feed от 283 независимых provider identities** с потоками HTTP/HTTPS/SOCKS4/SOCKS5. VPN-каталог содержит **271 HTTPS feed от 33 провайдеров** для OpenVPN, WireGuard, VLESS, VMess, Trojan, Shadowsocks, Hysteria2 и TUIC.

Аудит **10 сентября 2026 года** сопоставил канонический каталог с фактическими production-результатами и отдельно проверил новые URL с VPS. Пустой GProxyNet, HTML-страница CyberGateway, локальные loopback-порты KhaiNguyenDuc и ошибочно классифицированная MetaFetch subscription удалены из proxy-каталога. Пять feed размером 18–23 МБ также заменены: один из них содержал 1 193 021 уникальный endpoint и неизбежно нарушал ограничение `MaxProxiesPerSource`. Защитный лимит 10 МБ сохранён без исключений. MetaFetch добавлен в VPN-каталог, а два отсутствующих country-feed Au1rxx (AF и MU) удалены. Все девять новых proxy URL и перенесённый VPN feed проверены на доступность и разбираемое содержимое. Это URL/content-аудит, а не подмена полного сетевого прогона: CI и production release-gate повторяют настоящий collector и validation. Каноническая дата `LastAuditedOn` публикуется в admin diagnostics и Prometheus.
...
Проверка требует ровно 271 уникальных HTTPS URL от 33 провайдеров, непустую лицензию или условия публикации и хотя бы одну поддерживаемую конфигурацию в каждом ответе. В CI используется быстрый `-CatalogOnly`, а еженедельный source-audit выполняет живую сетевую проверку всех VPN-feed.
