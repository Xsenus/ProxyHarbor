...
Последний полный live-аудит: **10 августа 2026 года, 19:08 (Asia/Novosibirsk)**. Все **81/81** endpoint'ов от **50/50** провайдеров были успешно обработаны одним production-циклом на чистой PostgreSQL за 5,213 секунды без ошибок, пустых результатов, skip или усечения: 887 861 распознанная строка и 289 830 уникальных кандидатов после межисточниковой дедупликации. Следующая production validation-партия выполнила 1 600 объективных HTTPS-проверок без `Deferred`, нашла 6 рабочих адресов и без расхождений опубликовала текущий живой набор как JSON, XML, TXT и CSV. Аудит также повторно подтвердил, что предел 500 000 полностью обрабатывает крупные Rix4Uni, ProxRipper и Fyvri и не достигает общего лимита 500 000 кандидатов. Еженедельный `source-feed-audit` воспроизводит полный сбор и отдельную validation-партию. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures`, `LastItemCount` и признаки полноты, а работоспособность и текущая производительность validator наблюдаются независимо. Каноническая дата `LastAuditedOn` публикуется в diagnostics, React-панели и метрике `proxyharbor_builtin_catalog_audit_timestamp_seconds`.
