...
Последний полный production-аудит: **9 августа 2026 года**. Все **81/81** endpoint'ов были успешно обработаны одним циклом без ошибок и пустых результатов: 553 598 распознанных строк до межисточниковой дедупликации. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures` и `LastItemCount`, а работоспособность самих прокси всегда проверяет независимо.
