...
Последний полный live-аудит: **9 августа 2026 года**. Все **81/81** endpoint'ов были успешно обработаны одним production-циклом без ошибок, пустых результатов и усечения: 875 256 распознанных строк и 287 705 уникальных кандидатов после межисточниковой дедупликации. Аудит обнаружил, что прежний предел 100 000 тихо обрезал Rix4Uni, ProxRipper и Fyvri; проверенный предел 500 000 обработал их фактические 255 380, 148 674 и 226 246 записей полностью, не достигнув общего лимита 500 000 кандидатов. Следующий production validation-аудит проверил 1 000 адресов за 29,062 секунды (34,41 проверки/с), нашёл 6 Alive и не получил Deferred; публичная выдача показала те же 6 свежих прокси. Еженедельный `source-feed-audit` воспроизводит полный сбор, затем проверяет 250 реальных адресов и требует согласованную выдачу Alive-набора во всех четырёх форматах. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures`, `LastItemCount` и признаки полноты, а работоспособность и текущая производительность validator наблюдаются независимо.
