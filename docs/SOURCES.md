...
Скрипт запускает настоящий production-сбор, сохраняет машиночитаемый JSON-отчёт даже при раннем отказе и завершится ошибкой, если admin-команда не вернула завершённый цикл, хотя бы один активный источник не был заново обработан именно в этом цикле, его результат пуст/ошибочен/усечён, счётчики `CollectionRun` расходятся с состоянием feed'ов либо достигнут `MaxCandidatesPerRun`. Отдельный completeness-gate требует наличия и включённости всех 81 встроенных feed'ов и ровно 50 заявленных провайдеров, поэтому скрыть сломанный или неполный источник его отключением нельзя; пользовательские feed'ы не искажают эту проверку. Workflow `Source feed audit` повторяет аудит еженедельно и сохраняет отчёт и API-лог как GitHub Actions artifacts; отсутствие любого ожидаемого artifact также считается ошибкой. Основной CI запускает локальный HTTP contract-test и доказывает три ветви самого аудитора: принятие свежего согласованного цикла, отклонение stale evidence и сохранение отчёта при ранней HTTP-ошибке.
