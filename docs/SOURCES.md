...
Последний полный live-аудит: **10 августа 2026 года, 11:13 (Asia/Novosibirsk)**. Все **81/81** endpoint'ов от **50/50** провайдеров были успешно обработаны одним production-циклом на чистой PostgreSQL за 6,869 секунды без ошибок, пустых результатов, skip или усечения: 887 656 распознанных строк и 289 245 уникальных кандидатов после межисточниковой дедупликации. Следующая production validation-партия выполнила 250 объективных HTTPS-проверок без `Deferred`, нашла 4 рабочих адреса и без расхождений опубликовала их как JSON, XML, TXT и CSV. Аудит также повторно подтвердил, что предел 500 000 полностью обрабатывает крупные Rix4Uni, ProxRipper и Fyvri и не достигает общего лимита 500 000 кандидатов. Еженедельный `source-feed-audit` воспроизводит полный сбор и отдельную validation-партию. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures`, `LastItemCount` и признаки полноты, а работоспособность и текущая производительность validator наблюдаются независимо. Каноническая дата `LastAuditedOn` публикуется в diagnostics, React-панели и метрике `proxyharbor_builtin_catalog_audit_timestamp_seconds`.
