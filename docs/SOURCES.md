...
Последний полный production-аудит: **9 августа 2026 года**. Все **81/81** endpoint'ов были успешно обработаны одним циклом без ошибок и пустых результатов: 569 232 распознанных строки до межисточниковой дедупликации и 216 048 уникальных кандидатов. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures` и `LastItemCount`, а работоспособность самих прокси всегда проверяет независимо.
...
```powershell
./tools/Audit-SourceFeeds.ps1 -AdminKey 'значение ADMIN_API_KEY'
```

Скрипт запускает настоящий production-сбор и завершится ошибкой, если активный источник ничего не вернул или сообщил ошибку.
