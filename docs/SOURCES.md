...
Успешный `200` сохраняет bounded HTTP validators `ETag` и `Last-Modified` в строке источника, backup и restore. Следующий запрос передаёт их как `If-None-Match`/`If-Modified-Since`; `304 Not Modified` считается свежей успешной проверкой, сохраняет прежние count/truncation и не создаёт кандидатов или сетевой/CPU расход повторного body. Ответ `304` принимается только после прежнего успешного непустого результата и реально отправленного validator. При смене URL/protocol validators сбрасываются, а malformed или содержащий control characters ETag отклоняется fail-closed.
