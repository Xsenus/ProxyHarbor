...
Встроенный каталог содержит **198 HTTPS feed-endpoint от 75 независимых проектов и сервисов** с отдельными HTTP/HTTPS/SOCKS4/SOCKS5 потоками там, где провайдер их публикует.

Предыдущий полный end-to-end аудит базовых 81 endpoint выполнен **10 августа 2026 года, 23:50 (Asia/Novosibirsk)** без ошибок, пустых результатов, skip или усечения. **24 августа 2026 года** добавлены 17 URL шести провайдеров. **26 августа 2026 года** добавлены ещё 100 URL от 19 новых origin-владельцев: каждый адрес ответил HTTP 200 и вернул хотя бы один разбираемый `IP:port`. Это URL/live-аудит, а не подмена полного сетевого прогона: workflow теперь требует ровно **198/75** и перед production-релизом повторяет настоящий collector и validation. Каноническая дата `LastAuditedOn` публикуется в admin diagnostics и Prometheus.
