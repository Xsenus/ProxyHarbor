...
Дополнительный лёгкий preflight 10 августа 2026 года повторно подтвердил HTTPS-доступность и наличие proxy-like записей во всех **81/81** endpoint'ах. Он не заменяет указанную выше полную production-проверку parser/limits/deduplication/validation, поэтому каноническая дата `LastAuditedOn` остаётся датой полного аудита и явно публикуется в diagnostics, React-панели и метрике `proxyharbor_builtin_catalog_audit_timestamp_seconds`.
