...
Последний полный live-аудит: **9 августа 2026 года**. Все **81/81** endpoint'ов были успешно обработаны одним production-циклом без ошибок, пустых результатов и усечения: 875 256 распознанных строк и 287 705 уникальных кандидатов после межисточниковой дедупликации. Аудит обнаружил, что прежний предел 100 000 тихо обрезал Rix4Uni, ProxRipper и Fyvri; проверенный предел 500 000 обработал их фактические 255 380, 148 674 и 226 246 записей полностью, не достигнув общего лимита 500 000 кандидатов. Реальный validator также проверен контрольной партией из 1 000 адресов, а выдача живой выборки разобрана как корректные JSON, XML, TXT и CSV. Поскольку внешние бесплатные каталоги нестабильны, ProxyHarbor повторяет временные ошибки, сохраняет `LastError`, `LastSucceededAt`, `ConsecutiveFailures`, `LastItemCount` и признаки полноты, а работоспособность самих прокси всегда проверяет независимо.
