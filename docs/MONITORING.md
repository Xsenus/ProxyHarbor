...
| `ProxyHarborStaleProxyRetention` | устаревшие неарендованные и ни разу не работавшие Pending/Dead остаются более 30 минут | Проверить успешность collection, PostgreSQL delete и cluster lock; исторические адреса, которые когда-либо работали, намеренно не входят в эту метрику |
