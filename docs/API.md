...
на `free` получает 10 средних записей из разных стран — не premium-верхушку и не худший хвост. Для гостя тот же лимит привязан к IP. Следующая
...
```json
{
  "access": {
    ...
    "message": "Бесплатный доступ: 10 прокси среднего качества из разных стран раз в 10 минут. Для неограниченного доступа купите подписку.",
    "upgradeUrl": "/account"
  },
  "proxies": []
}
...
Публичный VPN-каталог и экспорт включают только полностью готовые записи: проверенный endpoint, определённую страну и импортируемый `connectionUri`. Неполные записи остаются в административном реестре и не показываются пользователю. Free-тариф получает 10 средних VPN, но ответ сохраняет полный `total`, поле `accessible`, `limited: true`, локализованное сообщение о подписке и `upgradeUrl`. Платная сессия получает обычную серверную пагинацию и экспорт до 5 000 URI за запрос.
...
Администратор управляет ролями аккаунта через `GET /api/v1/admin/users` и `PUT /api/v1/admin/users/{id}`. Реестр пользователей всегда постраничный (`page`, `pageSize`, максимум 100 записей) и поддерживает серверные фильтры `search` (имя, логин или почта), `activity=active|disabled` и `plan=free|pro|unlimited`. Total и bounded-страница читаются в общем `REPEATABLE READ`; пользователи, роли и подписки страницы формируются одним SQL reader без N+1 и без загрузки Identity-секретов. Удаление роли у последнего администратора запрещено. Коммерческий доступ и защита выдачи вынесены в отдельные реестры:
...
## POST `/api/v1/admin/backups/pools`

Атомарно создаёт пользовательский protection pool с включёнными S3 routes. Тело содержит `name`, `requiredVerifiedCopies`, `desiredVerifiedCopies`, `maxAttemptsPerCycle`, `overallDeadlineSeconds`, `failbackHealthyForSeconds` и массив `routes` (1–16 элементов: `destinationId`, `priority`, `role`, `activateDestination`). Для каждого маршрута разрешаются `put,verify,read`. Отключённое пользовательское назначение можно включить только явным `activateDestination=true`, если у него ещё нет маршрутов и копий; legacy S3 таким образом не включается. Сервер проверяет расшифровку credentials, S3-настройки и минимум `desiredVerifiedCopies` различных host/region failure domains. Ошибки конфигурации и недостаток независимых доменов дают `409`, неверная policy — `400`; ответ `201` содержит только ID/имя/версию policy и числа копий/маршрутов. Создание pool **не** меняет `BackupRouting__PoolId`/`BackupRouting__Enabled`, не обращается к S3 и не доказывает доступность provider или независимость аккаунтов с разными endpoint.
...
## POST `/api/v1/admin/backups/pools/{poolId}/routes/{destinationId}/activate`

Возвращает пользовательский S3-маршрут к новым PUT после draining. Тело `{ "expectedPolicyVersion": 4 }` обязательно. Сервер проверяет актуальную версию policy, разрешение PUT, включённость назначения и возможность расшифровать его credentials; несоответствие или legacy pool даёт `409`, отсутствие pool/route — `404`, некорректная версия — `400`, недоступная серверная защита credentials — `503`. Повторный вызов идемпотентен (`204`). Операция не обращается к S3, не проверяет реальные права или доступность provider, не меняет числовую policy и не переключает активный pool. Перед включением production routing выполните отдельный protocol canary и изолированный restore.
...
`GET/PUT /api/v1/admin/site-settings` доступны только администратору и возвращают полный редактируемый снимок. PUT требует точный набор кодов разделов и реквизитов, проверяет длины и форматы идентификаторов (`Yandex` numeric, GA4 `G-…`, VK Pixel), а при изменении cookie-текста или аналитики атомарно увеличивает `cookieConsentRevision`. Оферту, политику данных, отдельное согласие и политику cookies нельзя снять с публикации, но можно убрать из навигации.
...
Возвращает до 100 незаблокированных пользователей, уже написавших основному Telegram-боту. Поддерживает bounded-поиск `query`; `@Xsenus` сортируется первым и помечается `isDefault=true`.
