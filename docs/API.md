...
Администратор управляет ролями аккаунта через `GET /api/v1/admin/users` и `PUT /api/v1/admin/users/{id}`. Удаление роли у последнего администратора запрещено. Коммерческий доступ и защита выдачи вынесены в отдельные реестры:
