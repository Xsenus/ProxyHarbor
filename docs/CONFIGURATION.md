...
шлюза или перезапуск сервера не теряет платёж. ЮMoney остаётся webhook-only: API истории кошелька
...
самого платежа. Telegram Stars подтверждаются только доверенным `successful_payment` от Telegram.
