...
После canary оператор может создать отдельный пользовательский pool через `POST /api/v1/admin/backups/pools` с явным allowlist S3 routes; запрос атомарно включает только выбранные ранее выключенные назначения. Он не выполняет provider I/O и не меняет production routing flag/pool ID. Не выбирайте этот pool в `BackupRouting__PoolId` до проверки реальных прав, вместимости и независимого восстановления; `desiredVerifiedCopies=2` означает только целевую policy, а не доказательство двух уже проверенных копий.
