...
Маршрут пользовательского pool можно временно остановить и вернуть к новым PUT в админке либо через `POST /api/v1/admin/backups/pools/{poolId}/routes/{destinationId}/drain` и `POST /api/v1/admin/backups/pools/{poolId}/routes/{destinationId}/activate`, каждый раз передав актуальный `expectedPolicyVersion`. Подтверждение включения в UI предупреждает, что при активном pool снова начнутся PUT. Возврат проверяет локальную конфигурацию и зашифрованные credentials, но **не** выполняет S3 canary и сам по себе не подтверждает восстановимость копии. Legacy pool этими операциями не меняется.
...
По умолчанию destination routing выключен, поэтому ручной запуск сохраняет прежний `200`-контракт. После контролируемого включения `BackupRouting__Enabled=true` код ответа означает доказанную защиту: `200` только для `protected` или `degraded` (required quorum достигнут), `202` для durable `pending`, `503` для `unavailable`. Необязательный `BackupRouting__PoolId` выбирает заранее настроенный pool; пустое значение оставляет зарезервированный legacy pool, а несуществующий UUID останавливает backup до создания архива. Переключение требует отдельной проверки policy и маршрутов: этот параметр не создаёт destinations автоматически. Локальный staging не является внешней копией и сам по себе не делает backup защищённым. Поля `backupRunId`, `protectionState`, verified/required/desired и copy debt позволяют автоматизации отличить созданный ciphertext от подтверждённой внешней защиты.

В админке у каждой записи журнала backup доступен read-only разворот «Состояние защиты и копий». Он показывает текущую оценку по сохранённому policy snapshot и статус зарегистрированных внешних копий. Для legacy-записей или некорректного маршрута оценка намеренно неизвестна; интерфейс не выдаёт это за защищённый backup. Экран не раскрывает S3 locator, object key или credentials.
...
Для отдельного тестового S3 bucket есть ограниченный protocol canary `pwsh -File tools/Invoke-IsolatedS3Canary.ps1 -Bucket <test-bucket>` (без `-Execute` это dry-run без provider I/O). После `dotnet build ProxyHarbor.slnx -c Release` проще всего запустить `pwsh -File tools/Invoke-IsolatedS3Canary.ps1 -Bucket <test-bucket> -ConfirmBucket <test-bucket> -PromptForCredential -Execute`: появится защищённый запрос, в котором S3 Access Key вводится как имя пользователя, S3 Secret Key — как пароль. Ключи не передаются аргументами команды и не сохраняются в файл. На Windows остаётся альтернативный способ для автоматизации: `Get-Credential | Export-Clixml "$env:TEMP\proxyharbor-drill-credential.xml"`, затем запуск с `-CredentialPath "$env:TEMP\proxyharbor-drill-credential.xml" -ConfirmBucket <test-bucket> -Execute` и адресное удаление временного файла. Не отправляйте ключи повторно в чат. Canary создаёт небольшой синтетический PHB3 и подписанный sidecar только под `proxyharbor-drill/`, проверяет PUT+HEAD, GET+PHB3 authentication, запрет повторного conditional PUT, публикацию catalog, GET и его подпись, затем удаляет только свои verified objects и подтверждает отсутствие через HEAD. При versioned bucket скрипт удаляет точные подтверждённые versions, если provider возвращает их ID; при отсутствии version ID может остаться скрытая версия. Если очистка не подтверждена, скрипт выводит точные object keys для ручной проверки. Это не полный DR-дрилл: исходная БД, реальный backup encryption key и Data Protection key ring здесь не восстанавливаются. Раскрытые в сообщениях ключи перевыпускайте после проверки.

Полная последовательность проверки потери production-БД/VPS, включая независимый catalog/ключи, offline materialization, изолированную БД, sentinels, RPO/RTO и stop-критерии, приведена в [ISOLATED_DR_DRILL.md](ISOLATED_DR_DRILL.md). Ни protocol canary, ни локальные интеграционные тесты не заменяют её выполнения на настоящем provider.

Для первичной сверки старых `BackupRuns` с локальным volume запустите read-only команду `dotnet run --project src/ProxyHarbor.Restore -- inventory --backups-directory <absolute-existing-directory>`. Ей нужны `ConnectionStrings__Postgres` и, если пароль хранится отдельно, `SecretFiles__PostgresPassword`; результат — JSON в stdout. Команда использует только колонки, существующие в deployed v7, не меняет БД и файлы и не обращается к S3. Она отмечает missing/size mismatch, дубли имён, неуспешные runs, symlink-файлы, orphan-файлы и игнорируемые чужие `.phbackup`; недопустимые имена из БД не выводятся. Даже `name-size-match-unverified` означает лишь совпадение метаданных: без ciphertext hash, расшифровки, удалённого получения и пробного восстановления целостность и DR не доказаны. Вывод может содержать имена backup-файлов и ID runs, поэтому сохраняйте его только в защищённом операторском контуре.

При необходимости добавьте `--hash-ciphertext`: команда последовательно вычислит SHA-256 каждого канонического локального PHB3 и добавит `ciphertextHashes` к JSON. Ссылки и соседние файлы вне namespace не читаются. Этот хеш можно сравнить с **ранее независимо зафиксированным** значением; хеш, впервые вычисленный сейчас, сам по себе не удостоверяет целостность, полноту таблиц, возможность расшифровать архив или восстановить сервис. Режим может нагрузить диск, поэтому запускайте его в согласованное окно.
