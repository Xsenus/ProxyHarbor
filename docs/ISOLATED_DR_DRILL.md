...
Если GET первой копии оборвётся уже во время чтения тела, recovery очищает частичный candidate и пробует следующую разрешённую копию в оставшемся дедлайне. Ошибка записи на локальный recovery-диск не считается отказом provider и останавливает операцию. Этот переход подтверждён локальной fault-injection проверкой, но ещё требует real-provider drill с двумя независимыми назначениями.
