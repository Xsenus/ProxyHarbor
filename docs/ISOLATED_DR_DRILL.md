...
- До начала проверьте manifest version исходного **реального** архива: существующие production PHB3 v7 неполны и не могут дать `PASS` для полного DR. Для такого перехода требуется отдельно одобренный flag-off deploy нового writer, свежий v9 archive и независимая проверка его восстановления; synthetic S3 canary не повышает старый v7 до полного backup (EVID-091 в [storage evidence](../_codex_reports/storage-audit/STORAGE_EVIDENCE.md)).
