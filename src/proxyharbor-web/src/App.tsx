...
type BackupSettings = { enabled:boolean;intervalHours:number;retentionDays:number;historyRetentionDays:number;maxTelegramFileSizeMb:number;sendToTelegram:boolean;telegramBotTokenConfigured:boolean;telegramChatId?:string;encryptionConfigured:boolean;format:string }
...
type AccountProfile = { id: string; userName: string; email: string; displayName?: string; preferredLanguage: Language; createdAt: string; lastLoginAt?: string; roles: string[]; subscription?: { plan: string; status: string; startedAt: string; expiresAt?: string }; entitlements:{unlimitedProxyAccess:boolean;apiTokens:boolean};apiTokens:AccountApiToken[] }
...
export default function App() {
  ...
  return <div className="app-shell">
    {!adminOpen && <><header>
      ...
      <nav><a href="#catalog">{t('proxies')}</a><a href="#vpn-catalog">{t('vpn')}</a><a href="#api">API</a><LanguageSwitcher compact/><a className="admin-link" href="/login"><LockKeyhole size={15}/> {t('signIn')}</a></nav>
...
function AccountPage() {
  ...
  const revokeApiToken=async(id:string)=>{setTokenBusy(id);setError('');const response=await fetch(`${API}/api/v1/account/api-tokens/${id}`,{method:'DELETE',credentials:'include'});if(!response.ok){setError(await responseMessage(response,'Не удалось отозвать токен'));setTokenBusy('');return}setTokenBusy('');await loadProfile()}
  return <main className="account-page"><header><a className="brand" href="/"><span className="brand-mark"><Network/></span><span>Proxy<span>Harbor</span></span></a><div><LanguageSwitcher compact/><a href="/">{t('home')}</a>{profile?.roles.includes('Administrator') && <a href="/admin">{t('admin')}</a>}<button onClick={logout}><LogOut/>{t('logout')}</button></div></header><section className="account-container"><div className="admin-section-heading"><div><span className="kicker">PERSONAL ACCOUNT</span><h1>{t('profile')}</h1><p>{t('profileText')}</p></div></div>{error && <div className="admin-notice"><X/>{error}</div>}{notice && <div className="account-success"><Check/>{notice}</div>}
    ...
    <section className="admin-card api-token-card"><div className="card-heading"><div><span className="kicker">SECURE API ACCESS</span><h2>{t('apiTokens')}</h2><p>{t('apiTokenProfileHint')}</p></div><ShieldCheck/></div>{!profile?.entitlements.apiTokens?<div className="token-locked"><LockKeyhole/><span>{t('tokenRequiresSubscription')}</span></div>:<><div className="token-issue"><label htmlFor="token-name">{t('tokenName')}</label><div><input id="token-name" maxLength={80} value={tokenName} onChange={event=>setTokenName(event.target.value)}/><button className="primary-admin-button" disabled={tokenBusy==='issue'||!tokenName.trim()} onClick={()=>void issueApiToken()}><Plus/>{t('issueToken')}</button></div></div>{issuedToken&&<div className="issued-token" role="status"><div><ShieldCheck/><strong>{t('tokenShownOnce')}</strong><button className="icon-button" data-tooltip={t('copy')} aria-label={t('copy')} onClick={()=>void copyText(issuedToken)}><Copy/></button></div><code>{issuedToken}</code><button className="token-dismiss" onClick={()=>setIssuedToken('')}>{t('ready')}</button></div>}</>}
      <div className="api-token-list">{profile?.apiTokens.map(token=><article key={token.id} className={!token.active?'revoked':''}><span><b>{token.name}</b><code>ph_live_••••{token.displaySuffix}</code></span><small>{token.lastUsedAt?`${t('checked')}: ${timeAgo(token.lastUsedAt)}`:t('neverUsed')}</small><em>{token.active?t('active'):t('unavailable')}</em>{!token.revokedAt&&<button className="icon-button danger" data-tooltip={t('revokeToken')} aria-label={t('revokeToken')} disabled={tokenBusy===token.id} onClick={()=>void revokeApiToken(token.id)}><Trash2/></button>}</article>)}</div>
    </section>
...
function AdminTelegramPage(){
  ...
  const load=useCallback(async()=>{try{const [configResponse,paymentResponse]=await Promise.all([fetch(`${API}/api/v1/admin/telegram`,{credentials:'include'}),fetch(`${API}/api/v1/admin/payments`,{credentials:'include'})]);if(!configResponse.ok)throw new Error(await responseMessage(configResponse,'Настройки Telegram недоступны'));const value=await configResponse.json() as Partial<TelegramSettings>;const config={...value,automaticProductCodes:value.automaticProductCodes??[],starsPerCurrencyUnit:value.starsPerCurrencyUnit??1,starsRoundingStep:value.starsRoundingStep??5,effectiveProductStars:value.effectiveProductStars??value.productStars??{},avatarUrl:value.avatarUrl??'/api/v1/admin/telegram/avatar'} as TelegramSettings;setSettings(config);setDraft(config);if(paymentResponse.ok){const payment=await paymentResponse.json() as AdminPaymentSettings;setProducts(payment.products.filter(x=>x.enabled))}setError('')}catch(reason){setError(reason instanceof Error?reason.message:'Настройки Telegram недоступны')}},[])
  ...
  const save=async()=>{if(!draft)return;setBusy('save');setNotice('');const response=await fetch(`${API}/api/v1/admin/telegram`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:draft.enabled,updateMode:draft.updateMode,name:draft.name,description:draft.description,shortDescription:draft.shortDescription,supportText:draft.supportText,proxyFileMaxItems:draft.proxyFileMaxItems,webhookMaxConnections:draft.webhookMaxConnections,productStars:draft.productStars,automaticProductCodes:draft.automaticProductCodes,starsPerCurrencyUnit:draft.starsPerCurrencyUnit,starsRoundingStep:draft.starsRoundingStep,botToken:tokenValue.trim()||null})});if(!response.ok)setError(await responseMessage(response,'Telegram-бот не сохранён'));else{const value=await response.json() as TelegramSettings;setSettings(value);setDraft(value);setTokenValue('');setError('');setNotice('Профиль, команды, изображение, цены Stars и режим доставки настроены автоматически.')}setBusy('')}
  ...
  const automaticStars=(product:AdminPaymentProduct)=>{if(!draft)return 0;const raw=product.amountMinor/100*draft.starsPerCurrencyUnit;return Math.ceil(raw/draft.starsRoundingStep)*draft.starsRoundingStep}
...
function AdminPaymentsPage() {
  ...
  return <section className="admin-section payment-settings" aria-labelledby="admin-payments-title">
    ...
    {!settings?<div className="admin-card payment-settings-loading"><RefreshCw className="spin"/>Загружаем настройки…</div>:<>
      ...
      {tab==='tariffs'&&<section className="admin-card payment-products-settings"><div className="card-heading"><div><span className="kicker">ТАРИФЫ</span><h2>Сроки и ценовая политика</h2><p>Укажите цену одного дня. Остальные цены рассчитываются автоматически; скидка растёт вместе со сроком, год всегда дешевле ежедневной оплаты на 20%.</p></div></div><div>{[...settings.products].sort((a,b)=>a.durationDays-b.durationDays).map(product=><article key={product.code}><Toggle checked={product.enabled} onChange={checked=>updateProduct(product.code,{enabled:checked})} label="Доступен"/><label>Период<input disabled value={subscriptionPeriodName(product.durationDays)}/></label><label>Цена, ₽<input type="number" disabled={product.durationDays!==1} min="0.01" max="10000000" step="0.01" value={(product.amountMinor/100).toString()} onChange={event=>updateProduct(product.code,{amountMinor:Math.round(Number(event.target.value)*100)})}/></label><label>Скидка, %<input type="number" min="0" max="20" step="1" disabled={product.durationDays===1||product.durationDays===365} value={product.discountPercent} onChange={event=>updateProduct(product.code,{discountPercent:Number(event.target.value)})}/></label><div className="tariff-policy-summary"><b>{product.durationDays===1?'Базовая цена':money(product.amountMinor,product.currency)}</b>{product.savingsMinor>0&&<small><s>{money(product.fullDailyPriceMinor,product.currency)}</s> · экономия {money(product.savingsMinor,product.currency)}</small>}</div><label className="payment-description">Описание<input maxLength={300} value={product.description} onChange={event=>updateProduct(product.code,{description:event.target.value})}/></label></article>)}</div></section>}
...
function AdminBackupSettings({onError}:{onError:(message:string)=>void}){
  ...
  const load=useCallback(async()=>{
    try{
      const response=await fetch(`${API}/api/v1/admin/backups/settings`,{credentials:'include'})
      if(!response.ok)throw new Error(await responseMessage(response,'Настройки резервного копирования недоступны'))
      setDraft(await response.json() as BackupSettings)
  ...
  const save=async()=>{
    ...
    try{
      const response=await fetch(`${API}/api/v1/admin/backups/settings`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:draft.enabled,intervalHours:draft.intervalHours,retentionDays:draft.retentionDays,historyRetentionDays:draft.historyRetentionDays,maxTelegramFileSizeMb:draft.maxTelegramFileSizeMb,sendToTelegram:draft.sendToTelegram,telegramBotToken:token||null,telegramChatId:draft.telegramChatId||null,clearTelegramCredentials:false})})
      if(!response.ok)throw new Error(await responseMessage(response,'Не удалось сохранить настройки резервного копирования'))
      setDraft(await response.json() as BackupSettings);setToken('')
