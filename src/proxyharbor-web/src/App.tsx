...
type AdminPaymentProviderOperational = { state:'disabled'|'configuration_required'|'healthy'|'pending'|'awaiting_first_payment'|'webhook_attention'|'no_successful_payments'; attention?:string; totalOrders:number; pendingOrders:number; paidOrders:number; failedOrders:number; canceledOrders:number; refundedOrders:number; lastOrderAt?:string; lastPaidAt?:string; webhookRequired:boolean; directReconciliationSupported:boolean }
...
function providerOperationalLabel(provider:AdminPaymentProvider){const state=provider.operational?.state;if(state==='healthy')return `${provider.operational?.paidOrders??0} подтверждённых оплат`;if(state==='pending')return `Ожидают подтверждения: ${provider.operational?.pendingOrders??0}`;if(state==='webhook_attention')return 'Webhook требует проверки';if(state==='no_successful_payments')return 'Нет подтверждённых оплат';if(state==='awaiting_first_payment')return 'Ожидает первой оплаты';return provider.ready?'Реквизиты заполнены':'Требуется настройка'}
function providerOperationalTone(provider:AdminPaymentProvider){const state=provider.operational?.state;return state==='healthy'?'healthy':state==='webhook_attention'||state==='no_successful_payments'?'attention':''}
