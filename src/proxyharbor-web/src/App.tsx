...
function AdminPaymentsPage() {
  ...
  return <section className="admin-section payment-settings" aria-labelledby="admin-payments-title">
    ...
    {!settings?<div className="admin-card payment-settings-loading"><RefreshCw className="spin"/>Загружаем настройки…</div>:<>
      ...
      {tab==='providers'&&<ProviderCards providers={providers} onOpen={(code,showHelp)=>{setInvoices(null);setProviderHelpOpen(showHelp);setProviderOpen(code)}}/>}
      ...
      {providerOpen&&<ProviderDialog provider={providers.find(x=>x.code===providerOpen)!} invoices={invoices} busy={busy} helpInitiallyOpen={providerHelpOpen} onClose={()=>{setProviderOpen(null);setProviderHelpOpen(false)}} onUpdate={updateProvider} onSave={()=>void save()}/>}
    </>}
  </section>
}
...
function ProviderCards({providers,onOpen}:{providers:AdminPaymentProviderDraft[];onOpen:(code:string,showHelp:boolean)=>void}){return <section className="provider-card-grid">{providers.map(provider=><article className="admin-card provider-card" key={provider.code}><div className="provider-card-icon"><CreditCard/></div><div><strong>{provider.name}</strong><small>{provider.ready?'Реквизиты заполнены':'Требуется настройка'}</small></div><span className={`state-pill ${provider.enabled&&provider.ready?'active':''}`}>{provider.enabled?'Включён':'Выключен'}</span><div className="provider-card-actions"><button aria-label={`Настройки ${provider.name}`} onClick={()=>onOpen(provider.code,false)}><Settings2/>Настройки</button><button aria-label={`Как подключить ${provider.name}`} onClick={()=>onOpen(provider.code,true)}><HelpCircle/>Как подключить</button></div></article>)}</section>}

function ProviderDialog({provider,invoices,busy,helpInitiallyOpen,onClose,onUpdate,onSave}:{provider:AdminPaymentProviderDraft;invoices:InvoicePage|null;busy:boolean;helpInitiallyOpen:boolean;onClose:()=>void;onUpdate:(code:string,patch:Partial<AdminPaymentProviderDraft>)=>void;onSave:()=>void}){
  ...
  return <div className="source-editor-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="source-editor-modal provider-modal" role="dialog" aria-modal="true" aria-label={`Настройки ${provider.name}`}><div className="source-editor-heading"><div><span className="kicker">PAYMENT PROVIDER</span><h2>{provider.name}</h2><p>Реквизиты шлюза изолированы; сохранённые секреты никогда не возвращаются в браузер.</p></div><button className="icon-button" aria-label={`Закрыть настройки ${provider.name}`} onClick={onClose}><X/></button></div>
    <details className="provider-help" open={helpInitiallyOpen}><summary><HelpCircle/>Как подключить {provider.name}</summary><div><b>{help.accountPath}</b><ol>{help.steps.map(step=><li key={step}>{step}</li>)}</ol><p>{help.webhookNote}</p><a href={help.officialUrl} target="_blank" rel="noreferrer">Официальная документация <ArrowRight/></a></div></details>
    <div className="provider-modal-status"><Toggle checked={provider.enabled} onChange={checked=>onUpdate(provider.code,{enabled:checked})} label="Принимать платежи"/><span className={`state-pill ${provider.ready?'active':''}`}>{provider.ready?'Готов':'Не настроен'}</span></div>
