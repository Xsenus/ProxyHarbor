...
function AdminPaymentsPage() {
  ...
  return <section className="admin-section payment-settings" aria-labelledby="admin-payments-title">
    <div className="admin-section-heading"><div><span className="kicker">BILLING CONTROL</span><h1 id="admin-payments-title">Оплата</h1><p>Счета, тарифы и восемь изолированных платёжных шлюзов, включая ЮMoney и криптовалюту.</p></div><button className="primary-admin-button" disabled={!settings||busy} onClick={()=>void save()}><ShieldCheck/>{busy?'Сохраняем…':'Сохранить изменения'}</button></div>
...
function AdminAccessPage(){
  ...
  return <section className="admin-section access-admin-section" aria-labelledby="admin-access-title"><div className="admin-section-heading"><div><span className="kicker">TRAFFIC & ANALYTICS</span><h1 id="admin-access-title">Доступ и IP</h1><p>Контроль выдачи прокси, блокировки и first-party аналитика посетителей.</p></div></div><AdminTabs value={tab} onChange={changeTab} ariaLabel="Разделы доступа и IP" idPrefix="access" items={[["traffic","Выдача и блокировки"],["visitors","Посетители сайта"]]}/>{tab==='traffic'?<AdminAccessTrafficPage/>:<AdminSiteVisitorsPage/>}</section>
}
