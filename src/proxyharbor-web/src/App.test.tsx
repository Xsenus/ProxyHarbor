...
describe('ProxyHarbor UI', () => {
  ...
  it('downloads and safely deletes a backup from the paged registry', async () => {
    ...
    expect(await screen.findByRole('heading',{name:/Назначения и маршруты/})).toBeInTheDocument()
    const addS3=screen.getByRole('button',{name:'Добавить S3'})
    fireEvent.click(addS3)
    const provisionDialog=screen.getByRole('dialog',{name:'Добавить S3-назначение'})
    fireEvent.keyDown(provisionDialog,{key:'Escape'})
    expect(screen.queryByRole('dialog',{name:'Добавить S3-назначение'})).not.toBeInTheDocument()
    expect(addS3).toHaveFocus()
    ...
    const protectionButton = screen.getByRole('button',{name:/Показать защиту proxyharbor/})
    expect(protectionButton).toHaveAttribute('aria-expanded','false')
    fireEvent.click(protectionButton)
    expect(protectionButton).toHaveAttribute('aria-expanded','true')
    expect(await screen.findByText('Защита снижена')).toBeInTheDocument()
    expect(screen.getByText(/Подтверждено независимых копий: 1/)).toBeInTheDocument()
    expect(screen.getAllByText('S3 Нидерланды').length).toBeGreaterThan(0)
    fireEvent.click(protectionButton)
    expect(protectionButton).toHaveAttribute('aria-expanded','false')
    expect(screen.queryByText('Защита снижена')).not.toBeInTheDocument()
