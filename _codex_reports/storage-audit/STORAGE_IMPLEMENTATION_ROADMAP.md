...
- Status: `PARTIAL: read-only per-run protection/copy detail, paged destination/route overview, route drain/reactivation API and UI, write-only S3 registration and atomic custom pool/route provisioning now exist; editing numeric policy and production provider proof remain`; Codex.
