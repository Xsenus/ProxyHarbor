...
public sealed class VpnSourceCatalogHealthTests
{
    ...
    public void CompleteAuditedCatalogIsHealthyAndIgnoresCustomFeeds()
    {
        ...
        Assert.Equal(270, snapshot.ExpectedSources);
        Assert.Equal(new DateOnly(2026, 9, 14), snapshot.LastAuditedOn);
        Assert.Equal(270, snapshot.PresentSources);
        Assert.Equal(270, snapshot.EnabledSources);
    ...
    public void DisabledBuiltInFeedMakesCatalogIncomplete()
    {
        ...
        Assert.Equal(270, snapshot.PresentSources);
        Assert.Equal(269, snapshot.EnabledSources);
