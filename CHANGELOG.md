...
- Административные реестры доступа и посетителей теперь строят всю 30-дневную сводку одним conditional aggregate, используют её unique-IP count как точный `total` и выбирают только последнюю связь аккаунта для IP на стороне PostgreSQL. Один ответ обращается к `ProxyAccessBuckets` три раза вместо прежних шести-семи, а сводка, страница, аккаунты и активные правила читаются в едином retry-safe `REPEATABLE READ` snapshot.
