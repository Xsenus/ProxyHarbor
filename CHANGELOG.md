...
### Added
- Встроенный каталог расширен до 320 proxy-feed от 85 независимых провайдеров и 174 VPN-feed от 32 провайдеров: добавлены новые российские и зарубежные источники, а два недоступных `404` endpoint удалены после повторного live-аудита 1 сентября 2026 года.
...
- Документация синхронизирована с текущими 1138 backend- и 68 frontend-тестами, backup schema v7 и фактической политикой бесплатной выдачи без оценочных формулировок о качестве.
...
- Ошибка отдельного proxy-feed больше не стирает число и признак усечения последнего успешного результата; conditional HTTP validators отправляются только при наличии пригодного непустого состояния, поэтому повреждённая запись восстанавливается полным `200`-ответом вместо цикла `304`. Удалённые `XYZS996 GR` и `XYZS996 GQ` заменены проверенными `XYZS996 US` и `XYZS996 TH` без уменьшения каталога `320/85`; startup переносит строку последнего источника на новый URL с сохранением истории и немедленным полным опросом.
