...
- Воспроизводимый `Test-BuiltInSourceEndpoints.ps1` выполняет bounded parallel live-аудит 81 feed/50 технических владельцев без системного proxy и публикует JSON failures; network-free `-CatalogOnly` contract включён в CI/release. Три последовательных live-run 10 августа подтвердили 81/81 endpoint с `IP:port`, нулём ошибок и worst-case 3,110 мс.
