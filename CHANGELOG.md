...
- Proxy membership retention теперь удаляет не только `Dead`, но и давно не встречавшиеся неарендованные `Pending`, предотвращая неограниченный рост БД при длительной недоступности control endpoint; подтверждённые `Alive` и активные lease сохраняются, а diagnostics, React, Prometheus и проверяемый alert раскрывают отложенную очистку.
