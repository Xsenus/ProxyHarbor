...
- публичная keyset pagination; бесплатный потоковый экспорт выдаёт 10 прокси среднего качества раз в 10 минут, а активные Pro/Unlimited — до 50 000 строк за запрос;
...
Сервис намеренно не сохраняет и не публикует UUID, пароли, userinfo, query-параметры и приватные ключи из конфигураций. Для TCP endpoint проверяется только доступность публичного адреса и измеряется время установления соединения. UDP endpoint помечаются как метаданные без активной проверки: такая проверка не доказывает полноценную работоспособность VPN и не использует чужие credentials.

- `GET /api/v1/vpn` — постраничные очищенные VPN endpoint; по умолчанию только TCP-доступные.
