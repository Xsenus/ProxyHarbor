...
ProxyHarbor загружает 543 proxy-feed от 283 независимых провайдеров, нормализует и дедуплицирует адреса, проверяет их через настоящий proxy-туннель до доверенного TLS endpoint, измеряет задержку и отдаёт только свежие подтверждённые прокси через API и экспорты JSON, XML, TXT и CSV.
...
- 543 встроенных proxy-feed от 283 провайдеров и 271 VPN-feed от 33 провайдеров; операторский каталог: [docs/SOURCE_CATALOG.md](docs/SOURCE_CATALOG.md).
...
- администраторская публикация сайта: видимость разделов и отдельных реквизитов, обязательный первый cookie-выбор, а также opt-in Яндекс Метрика, Google Analytics 4 и VK Pixel без произвольного JavaScript и без загрузки до согласия;
...
```mermaid
flowchart LR
    A["543 feeds / 283 providers"] --> B["Bounded collector"]
...
ALERTMANAGER_WEBHOOK_TOKEN=REPLACE_WITH_A_DIFFERENT_32_CHAR_SECRET
```
